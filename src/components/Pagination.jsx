...
export const Pagination = ({ currentPage = 1, totalPages = 1, handleLeft, handleRight }) => {
  return (
    <Box display='flex' flexDirection=''flexWrap='wrap' justifyContent='center' gap='20px' p='20px'>
      { currentPage === totalPages || (currentPage === 1 && <ArrowForwardIcon h={10} w={8} onClick={handleRight} />) }
      ...
      { currentPage === totalPages && currentPage > 1 && <ArrowBackIcon h={10} w={8} onClick={handleLeft} />}
    </Box>
  )
}
